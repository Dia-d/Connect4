    ...
    for i in range(4):
        for j in range(4):
            if matrix[i][j] == matrix[i+1][j+1] == matrix[i+2][j+2] == matrix[i+3][j+3] == "X":
                print("PLAYER X WON")
                exit()
            elif matrix[i][j] == matrix[i+1][j+1] == matrix[i+2][j+2] == matrix[i+3][j+3] == "O":
                print("PLAYER O WON")
                exit()
    ...
    if i % 2 ==0:
        while True:
            ...
            if int(placement[0])==6:
                ...
                if placement in placement_matrix1:
                    print("already placed")
                    z=input("enter to continue")
                    break_check =1
                elif (str(int(placement[0])+1)+placement[1]) not in placement_matrix1:
